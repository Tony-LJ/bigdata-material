...
def oracle_sync_impala(jdbc_url,
    ...
    if is_distributed == 0:
        ...
        """
        execute_sqoop_command(jdbc_url,username,password,source_table,sink_table,sqoop_type,sqoop_param)
        logger.info("开始常规抽数")
...
if __name__ == '__main__':
    ...
    if pipeline_type == 'oracle_sync_impala':
        oracle_sync_impala(jdbc_url,username,password,source_table,sink_table,sqoop_type,sqoop_param,is_distributed,split_by,parallelism)
    ...
    print(f"数据同步Task执行耗时: {execution_time:.6f}秒")
    print(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> sqoop抽数结束!")
